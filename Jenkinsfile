pipeline {
    ...
    stage('Pushing to ECR') {
     ...
     stage('stop previous containers') {
         steps {
            sh 'docker ps -f name=mypython -q | xargs --no-run-if-empty docker container stop'
            sh 'docker container ls -a -fname=mypython -q | xargs -r docker container rm'
         }
       }
    ...
    stage('Docker Run') {
     steps{
         script {
                sh 'docker run -d -p 8096:5000 --rm --name mypython 913665488114.dkr.ecr.us-east-1.amazonaws.com/iamrepo:latest'
            }
      }
    }
    }
